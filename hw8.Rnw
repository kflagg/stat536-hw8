...
% $\delta^{13}C$ is the ratio of the plant's carbon-13 to carbon-12 ratio to a standard carbon-13 to carbon-12 ration, expressed as a permil...
...
$$
for the mean white pine series. Rather than using these constant values, the cut-offs in Figure 3 generally increase for larger lags, probably to account for the lower number of pairs used to estimate the sample autocorrelation at larger lags.
...
Mark's answer choices are:

\begin{itemize}

\item description/underestanding
