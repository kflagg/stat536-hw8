...
Four reasons for doing time series:
...
Because the authors' main focus seems to be on describing the trends over time, I would say the main reasons the authors' did time series modelling falls in to the Description/Understanding category. In the summary section, the authors emphasize that the residuals of an appropriate ARMA model will satisfy the independence assumption and that hypothesis tests yield biased results when ``significant" autocorrelation is present but not accounted for.

The remainder of the paper proceeds to desribe AR and MA models, look at visual diagnostics for these types of models, and discuss the trends that were estimated.
...
{\bf Doublas-fir $\Delta$ AR(2) models}
<<prob6.dar2>>=
...
@
{\bf which delta does he mean? $\delta_{p}$ or $\Delta$??}
...
@
The first plot of the simulation residuals shows a more apparent positive autocorrelation pattern, although not strong, as residuals at time points closer together are more similar in sign. There appears to be more extreme spikes either positive or negative occuring every 15 to 20 time points out.
...
{\bf Please verify this part.}

Since there are complex roots, the process has a quasi-perodic structure. The quasi-period is given by $\frac{2\pi}{\bigodot}$ where cos($\bigodot$) = $\frac{\phi_{1}}{2(-\phi_{2})^{-{\frac{1}{2}}}}$
{\bf but this will yield a negative square root -- thoughts}

Note that $\phi_{1}$ = 0.06 and $\phi_{2}$ = 0.32. The AR(2) process is stationary if:
