...
Monserud and Marshall use ARMA models to describe and understand the trends in carbon isotope ratios. They are interested in ``lagged physiological effects that persist for longer than the current year'' (p~1087) and their discussion section links the observed trends and dependence structure to biological theory. The bigger picture for this research is that scientists will need to accommodate dependency in models used for forecasting or exploring the relationships between carbon isotope levels and various predictors, but in this study the authors focus on explicating the trends and dependence structures that may be useful in such models.

%\begin{itemize}

%\item description/underestanding --- They're interested in ``lagged physiological effects that persist for longer than the current year''. Also ``Little is known about the stochastic properties of $\delta^{13}C$ over the lifetime of forest-grown trees.'' Their discussion links the observed trends and dependence structure to biological theory. They point out that the structures that best describe most of the trees in their sample are relatively simple, with one correlation parameter.

%\item accomodation --- No, the autocorrelation structure is their focus. They use only simple trends in time and don't have other predictors. But the bigger picture is that temporal dependence affects forecasting and inference so they want to explicate the structures that may be useful in modeling for various purposes.

%\item monitoring/control --- No?

%\item forecasting --- Again, bigger picture...
